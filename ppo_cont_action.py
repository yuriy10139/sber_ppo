...
if __name__ == "__main__":
    ...
    rewards_raw = torch.zeros((args.num_envs, args.num_steps)).to(device)
    ...
    rewards_average = deque(maxlen=3)
    rewards_average_max = -1e6
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if (
                args.set_target
                and iteration * args.num_steps + step == args.set_target_step
            ):
                rewards_average_max = -1e6
                rewards_average.clear()
                rewards_average.append(-1e6)

        for env_id in range(args.num_envs):
            rewards_raw[env_id] = torch.Tensor(
        ...
        writer.add_scalar(
            "charts/episodic_return", rewards_raw.sum() / args.num_envs, global_step
        )
        print(
            f"episode {iteration} return: {rewards_raw.sum() / args.num_envs}, rewards_average = {np.mean(rewards_average)}, rewards_average_max = {rewards_average_max}"
        )
